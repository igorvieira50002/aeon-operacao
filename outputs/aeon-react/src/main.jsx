...
function App(){return <main><header><div><p className="eyebrow">GRUPO VGD / AEON OPERACIONAL</p><h1>Clareza para agir melhor.</h1><Text>Mapa vivo de capacidade, memória, decisão e evidência.</Text></div><Badge color="teal">sistema validado</Badge></header><section className="metrics"><Card><Metric title="Capacidade ativa" value="5 / 7" delta="componentes operacionais" deltaType="increase"/></Card><Card><Metric title="Memória" value="Mem0" delta="Ollama + Qdrant local"/></Card><Card><Metric title="Observabilidade" value="Langfuse" delta="traces reais"/></Card><Card><Metric title="Governança" value="Humana" delta="efeitos externos protegidos"/></Card></section><section className="grid"><Card className="map-card"><div className="section-title"><div><h2>Mapa vivo do AEON</h2><Text>Arraste, aproxime e explore o sistema.</Text></div><Badge color="teal">React Flow</Badge></div><div className="flow"><ReactFlow nodes={nodes} edges={edges} fitView><Background color="#27343b" gap={24}/><Controls/><MiniMap nodeColor="#7be7d0"/></ReactFlow></div></Card><Card><div className="section-title"><div><h2>Ritmo operacional</h2><Text>Execuções registradas por dia</Text></div><Badge color="amber">local</Badge></div><AreaChart className="chart" data={data} index="day" categories={['exec']} colors={['teal']} showLegend={false} showGridLines={false}/><div className="log"><strong>Próxima ação</strong><p>Usar o dashboard como painel diário e ajustar os smoke tests do Promptfoo.</p><strong>Princípio</strong><p>Uma ação clara, uma evidência real e aprovação humana.</p></div></Card></section></main>}
